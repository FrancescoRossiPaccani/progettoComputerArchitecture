...
// ============================================================
// SPECIALIZED 3x3 WITH INTERIOR/BORDER SPLIT
// ============================================================
...
void processChannel<1>(uint8_t* __restrict in,
    ...
    auto process_scalar_pixel_clamped = [&](int y, int x) {
        int ym1 = max(y - 1, 0);
        int yp1 = min(y + 1, h - 1);
        int xm1 = max(x - 1, 0);
        int xp1 = min(x + 1, w - 1);
    ...
    if (w < 3 || h < 3) {
        for (int y = start_y; y < end_y; y++) {
            for (int x = 0; x < w; x++) {
                process_scalar_pixel_clamped(y, x);
            }
        }
        return;
    }
    ...
    for (int y = start_y; y < end_y; y++) {
        ...
        // bordo sinistro
        process_scalar_pixel_clamped(y, 0);

        // interno puro: x in [1, w-2]
        int x = 1;
        const int interior_end = w - 1;
        ...
        for (; x <= w - 33; x += 32) {
#if ENABLE_PREFETCH
            int pf = min(x + PREFETCH_DISTANCE_AVX, w - 2);
        ...
        // bordo destro
        process_scalar_pixel_clamped(y, w - 1);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    cout << "Errore medio sui pixel pre-filtro (Rumore): "
         << initial_error << " (MAE su scala 0-255)\n";
    ...
    if (radius == 1) cout << " [AVX2 SORTING NETWORK 3x3 INTERIOR/BORDER]\n";
    ...
    cout << "Errore medio sui pixel post-filtro (Migliorata): "
         << final_error << " (MAE su scala 0-255)\n";
