...
void processChannel(uint8_t* __restrict in, uint8_t* __restrict out, int w, int h, int kernel, int threshold, int start_y, int end_y, int bucket_size){
    int radius = kernel/2;
...
// ---------------- THREAD WORKER ----------------
void worker(Image& img, Image& out, int kernel, int threshold, std::atomic<int>& next_row, int end_y, int chunk_size, int bucket_size){
    int c = 0;
    int s = 0;
    while (true){
    ...
    for(int k = 0; k <= 1; k++){
        ...
        for(int t = 20; t>0; t--){
            ...
            for(int j = 0; j<10; j++){
                ...
                int div = 100;
                int chunk_size = 1;//std::max(1, img.height / (n_threads * div));
                auto start = chrono::high_resolution_clock::now();


                for(int i=0;i<n_threads;i++)
                {
                    threads.emplace_back(worker, ref(img), ref(out), kernel, threshold, ref(next_row), img.height, chunk_size, bucket_size);
                }
                ...
                cout<<"Thread: " << n_threads<<" \t|| Run: "<< j << " \t|| "<<"Tempo: " <<chrono::duration_cast<chrono::milliseconds>(end-start).count() <<" ms\n";
                std::this_thread::sleep_for(std::chrono::seconds(1));
            }
        }
    }
