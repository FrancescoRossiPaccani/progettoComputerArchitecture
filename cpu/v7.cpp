...
int main(int argc, char* argv[])
{
    ...
    int div = 2;
    int chunk_size = std::max(1, img.height / (n_threads * div));
    mutex mtx;
    int counter=0;
    ...
    {
        threads.emplace_back(worker, ref(img), ref(out), kernel, threshold, ref(next_row), img.height, chunk_size, bucket_size, ref(counter), ref(mutex));
    }
