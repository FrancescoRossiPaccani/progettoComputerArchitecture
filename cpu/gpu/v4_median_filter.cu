...
// A differenza della v3 e v4 precedente, calcoliamo la mediana su registri nativi
// in modo indipendente per ogni canale. Questo evita di far "aspettare" l'ALU 
// per il caricamento contemporaneo di R, G e B, massimizzando l'Instruction Level Parallelism (ILP).
...
__global__ void image_filtering(const uint8_t* __restrict__ img_r_in, 
    ...
    // Fast path: nessuna branch divergence se siamo lontani dai bordi
    if (x > 0 && x < img_w - 1 && y > 0 && y < img_h - 1) {
        ...
        // Slow path: clamping dei bordi per i thread perimetrali
        int ny0 = max(y - 1, 0);
        int ny1 = y;
...
int main(int argc, char* argv[])
{
    ...
    // Blocco 32x8 allineato alla lettura per warp
    dim3 block(32, 8);
